...
// Global variable for sorting, 0 = quick, 1 = insertion, 2 = BOTH
uint16_t sort = 0; 

//Global variable for ratings 1-5
uint16_t rating = 1;
...
void beginMode0() {
 ...
 // inital sort and rating buttons
  tft.fillRect(422, 0, 56, 159, TFT_WHITE);
  tft.drawRect(422, 0, 56, 159, TFT_RED);
  tft.setCursor(445, 85);
  tft.setTextSize(2);
  tft.setTextColor(TFT_BLACK, TFT_WHITE);
  tft.println(rating);

  tft.fillRect(422, 161, 56, 159, TFT_WHITE);
  tft.drawRect(422, 161, 56, 159, TFT_GREEN);

  char word[] = "QSORT";
  int cursor_incre = 200;

  for (int i = 0; i < 5; i++) {
  	tft.setCursor(445, cursor_incre);
  	tft.println(word[i]);
  	cursor_incre = cursor_incre + 16;
  }
...
void beginMode1() {
	...
	getAndSortRestaurants(curView, restaurants, &card, &cache, sort, rating);
...
void scrollingMap() {
  ...
  pinMode(XM, OUTPUT);
  	...
  	int16_t screen_y = map(touch.x, TS_MINY, TS_MAXY, DISP_HEIGHT-1, 0);

  	//Rating button
  	if (touch.z >= MINPRESSURE && touch.z <= MAXPRESSURE && screen_x > DISP_WIDTH && screen_y < DISP_HEIGHT/2) {
	...
	if (touch.z >= MINPRESSURE && touch.z <= MAXPRESSURE && screen_x <= DISP_WIDTH) {
		restaurant r;

  		lcd_image_draw(&edmontonBig, &tft, curView.mapX, curView.mapY, 0, 0, DISP_WIDTH, DISP_HEIGHT);
		tft.fillRect(curView.cursorX - CURSOR_SIZE/2, curView.cursorY - CURSOR_SIZE/2, CURSOR_SIZE, CURSOR_SIZE, TFT_RED);
		...
		for (int i = 0; i < NUM_RESTAURANTS; ++i) {
			...
			uint16_t convertedRating = max(floor((r.rating + 1)/2), 1);

			// only draw if entire radius-3 circle will be in the map display
			if (rest_x_tft >= 3 && rest_x_tft < DISP_WIDTH-3 &&  rest_y_tft >= 3 && rest_y_tft < DISP_HEIGHT-3 && convertedRating >= rating) {
...
void scrollingMenu() {
	...
	if (selectedRest == 21 && v > JOY_CENTRE + JOY_DEADZONE) {
		tft.fillScreen(TFT_BLACK);
		selectedRest = 0;
		screenNum++; 
		// Print the list of restaurants.
	...
	if (oldRest != selectedRest) {
		printRestaurant(oldRest + (screenNum*21));
		printRestaurant(selectedRest + (screenNum*21));
		delay(50); // so we don't scroll too fast
	}

	selectedRest = constrain(selectedRest, 0, NUM_RESTAURANTS-1);
